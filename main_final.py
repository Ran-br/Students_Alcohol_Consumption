...
class ATT_Estimator:
    ...
    def s_learner_att(self, model=GaussianProcessRegressor()):
        ...
        # print("G1", model.score(np.column_stack([self.X_treated, np.ones(self.X_treated.shape[0])]), self.y_treated))
        # print("G2", model.score(np.column_stack([self.X_treated, np.zeros(self.X_treated.shape[0])]), self.y_treated))
    ...
    def t_learner_att(self):
        ...
        # print('Accuracy of treated_model', treated_model.score(self.X_treated, self.y_treated))
        # print('Accuracy of control_model', control_model.score(self.X_treated, self.y_treated))
    ...
    def print_histogram(self, col_idx):
        ...
        cont_plt = plt.hist(self.X_control[:, col_idx], bins=100, label='Control')
        plt.legend()
...
def main():
    ...
    propensity_df = pd.DataFrame(data=[estimator1.all_propensity_score, estimator2.all_propensity_score], index=['data1', 'data2'])
    print(propensity_df)
